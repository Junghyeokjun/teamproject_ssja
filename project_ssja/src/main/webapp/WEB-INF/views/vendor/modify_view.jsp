	...
	<sec:authorize access="isAuthenticated()">
		<c:choose>
			<c:when test="${principal.auth == 'ROLE_VENDOR'}">
				...
				<main>
					<div id="main_container">
						...
						<form class="form"
							action="${pageContext.request.contextPath}/vendor/question/modify"
							method="post">
							<sec:csrfInput />
							<div class="input-group">
								<input type="hidden" class="form-control" name="bbcno"
									value="${modify_view.bbcno}"> <input type="hidden"
									class="form-control" name="bno" value="${modify_view.bno}">
							</div>
							<table class="table">
								<tr>
									<td colspan="2">
									...
									<td colspan="2">
										<div class="input-group">
											<textarea id="board_textarea" name="bcontent"
												class="form-control" rows="10" placeholder="내용을 입력하세요.">${modify_view.bcontent}</textarea>
										</div>
									</td>
								</tr>
								<tr>
									<td colspan="2">
										<div class="d-flex justify-content-between">
											<input type="submit" class="btn btn-danger customed-ssja"
												value="수정"> <a class="btn btn-primary customed-ssja"
												href="${pageContext.request.contextPath}/vendor/question/${modify_view.bbcno}">취소</a>
										</div>
									</td>
								</tr>
							</table>
			...
			<c:otherwise>
				<script type="text/javascript">
					$(document).ready(function() {
						alert("판매자 본인만 해당 페이지를 확인하실 수 있습니다. 먼저 로그인을 진행하시기 바랍니다.");
						window.location.href = "/login";
					});
				</script>
			</c:otherwise>
