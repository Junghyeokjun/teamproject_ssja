			...
			<c:when test="${principal.auth == 'ROLE_VENDOR'}">
				...
				<main>
					<div id="main_container">
						...
						<form class="form"
							action="${pageContext.request.contextPath}/vendor/question/write"
							method="post">
							<sec:csrfInput />
							<div class="input-group">
								<input type="hidden" class="form-control" name="bmno"
									value="${principal.memberNum}"> <input type="hidden"
									class="form-control" name="bbcno" value="${bcno}">
								<c:choose>
									<c:when test="${principal.isOAuth2User() == false}">
										<input type="hidden" class="form-control" name="bwriter"
											value="${principal.userInfo.m_Name}">
									</c:when>
									<c:otherwise>
										<input type="hidden" class="form-control" name="bwriter"
											value="${principal.getOAuth2Response().getNickName()}">
									</c:otherwise>
								</c:choose>
							</div>
							<table class="table">
									...
									<td colspan="2">
										<div class="input-group">
											<textarea id="board_textarea" name="bcontent"
												class="form-control" rows="10" placeholder="내용을 입력하세요."></textarea>
										</div>
									</td>
								</tr>
								<tr>
									<td colspan="2">
										<div class="d-flex justify-content-between">
											<input type="submit" class="btn btn-danger customed-ssja"
												value="입력"> <a class="btn btn-primary customed-ssja"
												href="${pageContext.request.contextPath}/vendor/question/${bcno}">취소</a>
										</div>
									</td>
								</tr>
							</table>
